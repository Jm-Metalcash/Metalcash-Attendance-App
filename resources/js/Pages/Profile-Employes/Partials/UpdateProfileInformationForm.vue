...
const form = useForm({
    ...
    role: (page.props.user.roles && page.props.user.roles.length > 0) ? page.props.user.roles[0].id : '',
});

// Créer une liste des rôles sans le rôle actuel de l'utilisateur
const otherRoles = computed(() => {
    return page.props.roles ? page.props.roles.filter(role => role.id !== form.role) : [];
});
</script>
...
<template>
    <section>
        <header>
            <h2 class="text-lg font-medium text-gray-900">Informations du profil de {{ page.props.user.name }}</h2>

            <p class="mt-1 text-sm text-gray-600">
                Mettez à jour les informations de profil et l'adresse e-mail de l'utilisateur.
            </p>
        </header>

        <!-- Formulaire de mise à jour -->
        <form @submit.prevent="form.patch(route('employees.update', page.props.user.id))" class="mt-6 space-y-6">
            <!-- Nom -->
            <div>
            ...
            <div>
                ...
                >
                    ...
                    <option v-if="page.props.user.roles && page.props.user.roles.length > 0" :value="page.props.user.roles[0].id">
            ...
            <div v-if="page.props.mustVerifyEmail && page.props.user.email_verified_at === null">
